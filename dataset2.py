...
class VideoDatasetSplitter:
    ...
    def load_data(self, dataset_config=None):
        """
        讀取所有指定結構的影片資料並轉為 DataFrame
        :param dataset_config: dict, 定義資料夾路徑與對應的標籤。若未提供則使用預設值。
        """
        # 如果使用者沒有自訂，就給一組預設的設定
        if dataset_config is None:
            dataset_config = {
                ...
                "1_fake/aigc": {"fine_label": "AIGC", "binary_label": "Fake"}
            }

        video_data = []
        
        # 🌟 使用 for 迴圈動態讀取字典裡的所有資料夾
        for folder_rel_path, labels in dataset_config.items():
            folder_path = os.path.join(self.base_dir, folder_rel_path)
            self._load_folder(
                folder_path=folder_path, 
                fine_label=labels["fine_label"], 
                binary_label=labels["binary_label"], 
