...
from util.dataset2 import VideoDatasetSplitter
import argparse
...
def process_dataframe(df, split_name):
    ...
    split_folder_path = Path("./pre_output") / f"{split_name}_img_list"
    ...
    for index, row in tqdm(df.iterrows(), total=len(df), desc=f"{split_name} 進度"):
        ...
        if target_image_path is not None and os.path.exists(target_image_path):
            ...
            safe_save_dir = Path("./pre_output") / split_folder_name 
...
def preprocess(split_name):
    ...
    splitter.save_to_csv(output_dir="./pre_output", split_name=split_name)